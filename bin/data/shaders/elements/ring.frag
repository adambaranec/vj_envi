...
st=st*2.-1.;
st.x*=aspect;
vec4 noise = hsv2rgba(.1,sin(time*.2)*.35+.35,_noise(vec3(st*10,time*.3)));
vec4 ring = hsv2rgba(0.,sin(time*.3)*.5+.5,sin(length(vec2(.0)-st)*8 - time*6));
fragColor = mix(noise,ring,step(.2,_luminance(ring.rgb)));
}